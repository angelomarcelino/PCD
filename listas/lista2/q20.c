...
int main(void) {
    ...
    * starts at: */
	...
	if (my_rank == 0) {
		...
		printf("of the integral from %.3f to %.3f = %.2f\n",
			   a, b, total_int);
	}
...
void Get_input(int my_rank, int comm_sz, double* a_p, double* b_p,
	...
	int size = 2 * sizeof(double) + sizeof(int) + 1;
	char buf[size];

	if (my_rank == 0) {
		printf("Enter a, b, and n\n");
		scanf("%lf %lf %d", a_p, b_p, n_p);
		printf("a = %.2lf, b =%.2lf, and n = %d\n", *a_p, *b_p, *n_p);

		int position = 0;
		MPI_Pack(a_p, 1, MPI_DOUBLE, buf, size, &position, comm);
		MPI_Pack(b_p, 1, MPI_DOUBLE, buf, size, &position, comm);
		MPI_Pack(n_p, 1, MPI_INT, buf, size, &position, comm);
		MPI_Bcast(buf, size, MPI_PACKED, 0, comm);
	} else {
		MPI_Bcast(buf, size, MPI_PACKED, 0, comm);
		int position = 0;
		MPI_Unpack(buf, size, &position, a_p, 1, MPI_DOUBLE, comm);
		MPI_Unpack(buf, size, &position, b_p, 1, MPI_DOUBLE, comm);
		MPI_Unpack(buf, size, &position, n_p, 1, MPI_INT, comm);
	}
